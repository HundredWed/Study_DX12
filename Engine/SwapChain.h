...
//요약하자면 화면을 그릴 때 어떤 식으로 그려줄지 정하는 클래스
class SwapChain
{
public:
	...
	ComPtr<ID3D12Resource> GetRenderTarget(int32 index) { return _renderBuffer[index]; }

	ComPtr<ID3D12Resource> GetBackRTVBuffer() { return _renderBuffer[_backBufferIndex]; }
...
private:
	...
	uint32					_backBufferIndex = 0;// 현재 작업중인 버퍼 인덱스
};
