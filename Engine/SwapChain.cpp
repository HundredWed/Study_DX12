...
void SwapChain::CreatSwapChain(const WindowInfo& info, ComPtr<IDXGIFactory> dxgi, ComPtr<ID3D12CommandQueue> cmdQueue)
{
	...
	for (int32 i = 0; i < SWAP_CHAIN_BUFFER_COUNT; i++)
		_swapChain->GetBuffer(i, IID_PPV_ARGS(&_renderBuffer[i]));
}
...
void SwapChain::CreatRTV(ComPtr<ID3D12Device> device)
{
	...
	// CBV(ConstantBufferView), SRV(ShaderResourceView), UAV(UnorderedAccessView)를 전부 통합관리
	...
	for (int i = 0; i < SWAP_CHAIN_BUFFER_COUNT; i++)
	{
		...
		device->CreateRenderTargetView(_renderBuffer[i].Get(), nullptr, _rtvHandle[i]);
	}
